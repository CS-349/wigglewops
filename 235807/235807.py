        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product == "ASH_COATED_OSMIUM":
                ...
                if bba:
                    ...
                    # Bootstrap fair value intercept from first observation
                    if "pepper_intercept" not in saved:
                        saved["pepper_intercept"] = mid - PEPPER_SLOPE * timestamp

                    fair_value = saved["pepper_intercept"] + PEPPER_SLOPE * timestamp

                    # Never cross fair value
                    max_bid = math.ceil(fair_value) - 1
                    min_ask = math.floor(fair_value) + 1

                    bid_price = min(bot_bid + 1, max_bid)
                    ask_price = max(bot_ask - 1, min_ask)
